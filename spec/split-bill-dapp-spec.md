...
**Tagline:** *"Chia bill nhóm có người ở nhiều chain — trả thẳng không cần nhờ ai trả hộ, và ai trả gì đều minh bạch, kiểm tra được."*
...
3. **Chưa có, cần cross-chain:** hệ thống check balance trên chain đã chọn trước khi cho ký (chặn lỗi sớm) → ký `depositForBurn` (kèm hookData) → đợi attestation (8–20s, Fast Transfer) → ký tiếp `receiveMessage` trên Arc (hoặc tự động nếu có relayer ở phase sau) → USDC mint + ghi nhận thanh toán atomic trong 1 tx.
...
`BurnMessageV2` của CCTP V2 mang theo `messageSender` (ví đã gọi `depositForBurn` ở chain nguồn) + `amount`, được Circle attest — không giả mạo được. Người trả encode `billId` (+ `shareId` nếu Assigned mode) vào `hookData` lúc gọi `depositForBurn`.

**Điểm cần sửa lại so với mô tả trước đây:** để Hook thực sự fire được, `mintRecipient` của giao dịch CCTP phải là **địa chỉ contract Bill**, không phải wallet cá nhân của organizer — vì hàm `handleReceiveFinalizedMessage` được gọi trên chính địa chỉ nhận mint. Nghĩa là với đường cross-chain, USDC **có chạm vào contract** một khoảnh khắc, ngay trong cùng 1 transaction:
...
function handleReceiveFinalizedMessage(
    ...
    (uint256 amount, bytes32 messageSender, bytes memory hookData) = decode(messageBody);
    ...
    usdc.transfer(bills[billId].organizer, amount); // forward ngay trong cùng tx — không giữ lại
}
```

Vì `transfer` forward tới organizer xảy ra atomic trong đúng transaction nhận mint, không có khoảng thời gian "giữ tiền" thật sự — đúng tinh thần "no custody" đã chốt, chỉ khác về kỹ thuật so với đường trực tiếp (chạm contract 1 dòng code, không phải giữ qua nhiều block).

**Chống double-spend:** `MessageTransmitter` tự track nonce theo bitmap, 1 message chỉ xử lý đúng 1 lần — không cần code thêm. Tuy nhiên, nonce KHÔNG chặn được trường hợp 2 burn riêng biệt (2 nonce khác nhau) cùng trỏ vào 1 billId/shareId — `AlreadyPaid` là lớp bảo vệ duy nhất cho case này, bắt buộc có ở Phase 3.
...
1. **CCTP Hook riêng** — test decode/validate/reject, tách biệt khỏi Bill thật (rủi ro nhất, làm trước). *Chạy được khi:* burn thật từ Base Sepolia, Hook decode đúng `messageSender`+`amount`, pass case đúng / reject case sai.
2. **Bill contract lõi**, chỉ trả trực tiếp trên Arc, cả 2 mode. *Chạy được khi:* tạo bill, vài wallet đã có USDC trên Arc tự `pay()`, trạng thái đúng.
3. **Gộp Phase 1+2** thành Bill thật, end-to-end 2 đường trả. *Chạy được khi:* 1 bill, 1 người trả trực tiếp + 1 người trả cross-chain, cả 2 lên đúng trạng thái, không cần gọi tay qua console.
4. **Frontend MVP** (2 chain đã chứng minh). *Chạy được khi:* tạo bill qua UI, share link, trả bằng 1 trong 2 đường, thấy trạng thái đổi trên UI.
...
*Spec này tổng hợp từ buổi Product Discovery — mọi quyết định đã qua xác nhận trực tiếp, trừ phần đánh dấu ở mục 2 (cách đếm X/N) là đề xuất cần được xác nhận hoặc sửa lại nếu không đúng ý.*
